  ...
  return (
    <div className="grid gap-8 xl:grid-cols-12">
      <section className="border border-outline-variant bg-surface-container-lowest p-5 xl:col-span-8 lg:p-7">
        ...
        <div className="relative h-64 border-b border-l border-outline-variant bg-surface-container-low p-2 sm:h-100 sm:p-3">
      ...
      <aside className="flex flex-col gap-8 xl:col-span-4">
        ...
        <div className="grid grid-cols-1 border border-outline-variant bg-surface-container-lowest sm:grid-cols-2">
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-wrap items-end justify-between gap-6 border-b border-outline-variant pb-5">
        <div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Current basket</p><p className="mt-2 font-mono text-4xl font-bold text-primary">{formatInr(total)}</p></div>
...
export function MarketDataPage({ data }: PageProps) {
  ...
  return <div className="space-y-12"><div className="grid border-y border-outline-variant sm:grid-cols-2 md:grid-cols-4">{indicators.map((indicator) => <Metric key={indicator.label} label={indicator.label} value={indicator.value} detail={indicator.change} />)}</div><section className="grid gap-8 lg:grid-cols-[.75fr_1.25fr]"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">02 / Regional distribution</p><h2 className="mt-3 text-3xl font-bold text-primary">Relative Inflation Map</h2><p className="mt-4 text-sm leading-6 text-muted-foreground">Explore regional variance against the national average. Search the monitored state set.</p></div><div className="border border-outline-variant bg-surface-container-lowest p-4 sm:p-5"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search regions..." className="mb-3 min-h-11 w-full border-b border-outline-variant bg-transparent px-2 py-3 font-mono text-sm outline-none placeholder:text-muted-foreground focus:border-primary" aria-label="Search regions" />{regions.map((region, index) => <div key={region} className="flex items-center justify-between gap-3 border-b border-outline-variant py-4"><span className="font-mono text-xs text-primary sm:text-sm">{String(index + 1).padStart(2, "0")} {region}</span><span className={index < 3 ? "font-mono text-xs text-destructive sm:text-sm" : "font-mono text-xs text-secondary sm:text-sm"}>{index < 3 ? `+${45 - index * 13}` : `-${18 + index * 7}`} bps</span></div>)}{regions.length === 0 && <p className="py-6 text-sm text-muted-foreground">No monitored regions match your search.</p>}</div></section></div>;
}
...
function Metric({ label, value, detail, tone = "default" }: { label: string; value: string; detail?: string; tone?: "default" | "secondary" | "muted" }) { return <div className="border-b border-outline-variant p-4 last:border-b-0 sm:border-r sm:p-5 sm:last:border-b-0 md:border-b-0"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">{label}</p><p className={`mt-3 font-mono text-2xl font-bold sm:text-3xl ${tone === "secondary" ? "text-secondary" : tone === "muted" ? "text-muted-foreground" : "text-primary"}`}>{value}</p>{detail ? <p className="mt-1 font-mono text-[10px] uppercase text-muted-foreground">{detail}</p> : null}</div>; }